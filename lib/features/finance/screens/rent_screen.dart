...
class RentScreen extends ConsumerWidget {
        ...
        data: (records) {
          if (records.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.receipt_long_outlined, size: 64, color: AppTheme.neutral300),
                  const SizedBox(height: 16),
                  const Text('Belum ada catatan sewa',
                      style: TextStyle(fontSize: 16, color: AppTheme.neutral500)),
                  const SizedBox(height: 8),
                  const Text('Atur biaya sewa dan WiFi di Kelola Rumah',
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) => _RentMonthCard(
              ...
              onMemberTap: (memberName) => _showPaymentSheet(context, ref, records[index], memberName),
            ),
          );
        },
      ),
  ...
  void _showPaymentSheet(BuildContext context, WidgetRef ref, RentRecord record, String memberName) {
    final payment = record.payments.firstWhere((p) => p.memberName == memberName);
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Pembayaran Sewa + WiFi',
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Rp${_formatRupiah(record.perMemberAmount)}',
                      style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 24, color: AppTheme.primary)),
                  const SizedBox(height: 4),
                  Text('Sewa Rp${_formatRupiah(record.totalRent)} + WiFi Rp${_formatRupiah(record.totalWifi)}',
              ...
              child: ElevatedButton.icon(
                onPressed: () {
                  ref.read(rentRepositoryProvider).uploadProof(
                    record.year, record.month, memberName,
                    'mock_photo_url_${memberName}',
                  );
                  ref.invalidate(rentHistoryProvider);
                  Navigator.pop(ctx);
            ...
            child: Row(
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('${monthNames[record.month]} ${record.year}',
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (record.isPaid ? AppTheme.success : AppTheme.warning).withOpacity(0.1),
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: Row(
                    children: [
                      const Text('Per orang', style: TextStyle(fontSize: 13, color: AppTheme.neutral500)),
                      const Spacer(),
                      Text('Rp${_formatRupiah(record.perMemberAmount)}',
...
class _MemberPaymentTile extends StatelessWidget {
  ...
  const _MemberPaymentTile({required this.payment, required this.onTap});
        ...
        child: Row(
          children: [
            CircleAvatar(
              ...
              child: Text(payment.memberName[0],
                  style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: AppTheme.primary)),
            ),
            const SizedBox(width: 8),
            Text(payment.memberName, style: const TextStyle(fontSize: 13)),
            const Spacer(),
            Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                if (payment.proofPhoto != null)
                  Padding(
                    ...
                    child: Icon(Icons.image_rounded, size: 14, color: AppTheme.primary),
                  ),
                Icon(
                  payment.isPaid ? Icons.check_circle_rounded : Icons.radio_button_unchecked_rounded,
                  color: payment.isPaid ? AppTheme.success : AppTheme.neutral400,
                  size: 18,
                ),
              ],
            ),
